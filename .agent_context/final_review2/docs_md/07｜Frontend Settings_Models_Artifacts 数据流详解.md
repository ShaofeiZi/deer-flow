...
```mermaid
flowchart TD
  Upload[用户上传文件] --> UploadAPI[Gateway uploads]
  UploadAPI --> Human[human message additional_kwargs.files]
  Human --> UploadsMW[UploadsMiddleware 注入上下文]

  Agent[agent present_files] --> State[ThreadState.artifacts]
  State --> ChatBox[ChatBox 读取 thread.values.artifacts]
  ChatBox --> Provider[ArtifactsProvider]
  Provider --> Trigger[ArtifactTrigger]
  Provider --> List[ArtifactFileList]
  List --> Detail[ArtifactFileDetail]
  Detail --> ArtifactURL[/api/threads/:threadId/artifacts/:path]

  ToolCall[write_file tool call] --> Temp[write-file: 临时预览]
  Temp --> Detail
```
