...
<callout emoji="💡">
**图解目标：**补充用户文件、工具写入和前端产物展示之间的关系图，突出虚拟路径和安全边界。
</callout>
...
```mermaid
flowchart LR
  UserFile[用户上传文件] --> UploadAPI[uploads router]
  UploadAPI --> UploadDir[thread uploads]
  UploadDir --> UploadMW[UploadsMiddleware]
  UploadMW --> Agent[Agent context]
  Agent --> SandboxTools[read/write/bash tools]
  SandboxTools --> Workspace[thread workspace]
  SandboxTools --> Outputs[thread outputs]
  Outputs --> ArtifactsState[ThreadState artifacts]
  ArtifactsState --> ArtifactAPI[artifacts router]
  ArtifactAPI --> Frontend[Artifacts Panel]
  Workspace --> Safety[virtual path safety]
  Outputs --> Safety
```

| 读图对象 | 图里怎么看 | 维护入口 |
|-|-|-|
| 模块职责 | uploads 负责输入，sandbox 负责执行，artifacts 负责输出展示 | uploads.py、sandbox/tools.py、artifacts.py |
| 数据流 | 文件从上传目录进上下文，产物从 outputs 回前端 | UploadsMiddleware、present_files |
| 风险点 | 路径穿越、主动内容、host bash 都在安全边界处理 | sandbox/security.py、artifacts router |
...
| 重点代码 | 重点代码：`/Users/bytedance/deer-flow/backend/packages/harness/deerflow/sandbox/tools.py`、`/Users/bytedance/deer-flow/backend/app/gateway/routers/uploads.py`、`/Users/bytedance/deer-flow/backend/app/gateway/routers/artifacts.py`。 |
