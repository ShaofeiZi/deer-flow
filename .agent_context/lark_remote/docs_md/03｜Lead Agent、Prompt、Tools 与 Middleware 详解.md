...
<callout emoji="💡">
**图解目标：**把模型、prompt、tools、skills、middleware 和 state_schema 的依赖关系画成组装图，降低读者理解门槛。
</callout>
...
```mermaid
flowchart TD
  Config[RunnableConfig + AppConfig] --> Model[resolve model]
  Config --> Skills[load enabled skills]
  Config --> Tools[get available tools]
  Skills --> Prompt[build prompt sections]
  Skills --> ToolPolicy[allowed tools filter]
  Tools --> ToolPolicy
  ToolPolicy --> Deferred[deferred tools]
  Config --> MW[build middleware chain]
  Prompt --> Agent[create_agent]
  Model --> Agent
  Deferred --> Agent
  MW --> Agent
  State[ThreadState schema] --> Agent
```

| 读图对象 | 图里怎么看 | 维护入口 |
|-|-|-|
| 模块职责 | lead_agent 是组装产物，不是单个函数逻辑 | agents/lead_agent/agent.py |
| 数据流 | 配置先决定模型、技能、工具，再进入 create_agent | prompt.py、tools.py、middlewares |
| 阅读路径 | 先看 make_lead_agent，再看 prompt/tools/middleware 分支 | 03 章节 + agent.py |
...
| 重点代码 | 重点代码：`/Users/bytedance/deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py`、`/Users/bytedance/deer-flow/backend/packages/harness/deerflow/agents/lead_agent/prompt.py`。 |
