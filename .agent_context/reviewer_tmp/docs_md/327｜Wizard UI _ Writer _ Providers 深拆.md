...
<callout emoji="💡">
**设计目的：**Wizard UI/Writer/Providers 把终端输入、provider 目录和配置落盘分开：`ui.py` 只负责交互，`providers.py` 只描述候选项，`writer.py` 才负责合并 `.env` 与 `config.yaml`。
</callout>

| 维度 | 说明 |
|-|-|
| 收益 | 新增 provider 或调整写盘逻辑时边界清晰，不需要改动所有 step。 |
| 代价 | 一次配置结果跨 `setup_wizard.py`、step result、provider extra_config 和 writer 合并逻辑，需要按数据流追。 |
...
| 阅读路径 | 先看 `providers.py` 数据结构，再看 `ui.py` 如何收集选择，最后看 `writer.py` 如何保留已有注释并更新 key。 |
